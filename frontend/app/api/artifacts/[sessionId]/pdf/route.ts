...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${apiBase}/artifacts/${sessionId}/pdf`;
    ...
    return new NextResponse(pdfBlob, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${sessionId}_executive_brief.pdf"`,
        "Cache-Control": "no-cache",
      },
    });
  } catch (error) {
    console.error("[artifacts/pdf] Error:", error);
    return NextResponse.json(
